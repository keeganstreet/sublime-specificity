...
(function () {
	...
	program
		.version('0.0.1')
		.option('-s, --selectors <string>', 'Comma or line-break separated list of CSS selectors', list)
		.parse(process.argv);
	...
	for (i = 0, len = program.selectors.length; i < len; i += 1) {
		spec = specificity.calculate(program.selectors[i])[0];
		output += spec.selector + ' - [' + spec.specificity + ']';
		if (i < len - 1) {
			output += '\n';
		}
	}
