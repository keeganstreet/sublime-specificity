...
(function () {
  ...
  var program = require('commander'),
    ...
    i, len, sel, res, output = '';

  // Initialise CLI
  program
    .version('0.0.1')
    .option('-s, --selectors <string>',
      'Comma or line break separated list of CSS selectors',
      parseList)
    .parse(process.argv);

  // Loop through each selector and calculate its specificity
  for (i = 0, len = program.selectors.length; i < len; i += 1) {
    sel = program.selectors[i];
    res = specificity.calculate(sel);
    output += sel + ' - [' + res[0].specificity + ']';
